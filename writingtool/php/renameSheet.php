...
if (mysqli_num_rows($gettingUID) == 1) {
    ...
    if (mysqli_num_rows($accessingStoryDB) == 1) {
        ...
        $updatingSheetName = mysqli_query($storyDbConn, "RENAME TABLE story_" . $chapter . " TO story_" . $chap . "_" . $newName . "");

        if ($updatingSheetName) {
            echo json_encode(array('msg' => "New Name Updated Successfully", "success" => true, 'sql' => $sql));
        } else {
